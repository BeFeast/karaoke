...
def test_merge_splices_word_tags_keeps_lrclib_line_tags():
    ...
    merged, word_timing, *_ = merge_lrclib_word_tags(synced, aligned)
...
# ---------------------------------------------------------------------------
# merge coverage counts (#237): eligible candidates vs. merged lines
# ---------------------------------------------------------------------------
def test_merge_reports_eligible_and_merged_counts():
    """The result reports how many single-tag candidate lines were eligible for
    word timing and how many actually merged — the #237 coverage signal. Multi-
    tag lines (never mergeable) are excluded from the eligible count."""
    synced = (
        "[00:10.00]alpha beta\n"          # merges
        "[00:20.00]gamma delta\n"         # merges
        "[00:30.00]epsilon zeta\n"        # aligner drops it → plain but eligible
        "[00:05.00][00:40.00]chorus line"  # multi-tag → NOT an eligible candidate
    )
    aligned = (
        "[00:10.05]<00:10.05>alpha <00:10.6>beta <00:11.0>\n"
        "[00:20.05]<00:20.05>gamma <00:20.6>delta <00:21.0>"
    )
    result = merge_lrclib_word_tags(synced, aligned)
    assert result.merged_any is True
    assert result.merged == 2
    assert result.eligible == 3  # three single-tag text lines; multi-tag excluded


def test_merge_counts_eligible_even_when_nothing_merges():
    """Every single-tag candidate counts as eligible even when drift rejects it,
    so coverage reflects the true mismatch — the wrong-text signature (#237)."""
    synced = "[00:10.00]alpha beta\n[00:20.00]gamma delta"
    aligned = (
        "[00:15.00]<00:15.00>alpha <00:15.5>beta <00:16.0>\n"  # drift 5 s → reject
        "[00:25.00]<00:25.00>gamma <00:25.5>delta <00:26.0>"   # drift 5 s → reject
    )
    result = merge_lrclib_word_tags(synced, aligned)
    assert result.merged_any is False
    assert (result.eligible, result.merged) == (2, 0)
    assert result.merged_lrc == synced  # byte-exact fallback


def test_merge_no_aligned_reports_zero_counts():
    """No aligned LRC → zero eligible/merged (nothing was attempted), byte-exact
    body, flag False — the gate treats this as 'no alignment', not a mismatch."""
    synced = "[00:10.00]alpha beta\n[00:20.00]gamma delta"
    for aligned in (None, "", "   \n  "):
        result = merge_lrclib_word_tags(synced, aligned)
        assert result.merged_any is False
        assert (result.eligible, result.merged) == (0, 0)
        assert result.merged_lrc == synced
