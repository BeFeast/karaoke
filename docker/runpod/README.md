...
without flagging it on issue #33. To roll the endpoint onto the r7 image,
bump `TEMPLATE_SPEC["imageName"]` to `…cuda12.4-r7` and re-run: the script
detects the image change and flushes warm/standby workers (`workersMax`
bounce to 0 → drain → restore) so the next job cold-pulls r7 instead of a
stale r6 worker (#75). The script prints a final
