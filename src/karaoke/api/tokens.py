...
Mint is restricted to ``clerk_user`` and ``machine_bearer``: minting escalates
the caller into the ``extension_token`` state, which is a cookie-writer state
(``COOKIE_WRITER_STATES`` in routes.py), and trusted-LAN is deliberately
excluded from cookie writes (#73) — so LAN-anon mint would be a privilege
escalation. An ``extension_token`` actor cannot mint either (no
token-begets-token); ``machine_bearer`` mint replaces the manual DB-insert
operator path.
